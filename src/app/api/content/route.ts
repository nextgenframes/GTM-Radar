...
export async function POST(request: Request) {
  ...
  try {
    ...
    const quickAiResult = await aiContent(idea, "");
    if (quickAiResult) {
      return NextResponse.json({ content: quickAiResult, isDemoFallback: false });
    }

    try {
      const research = await getResearchData(idea);
      const aiResult = await aiContent(idea, research.corpus);
      return NextResponse.json({ content: aiResult ?? buildContent(idea, research.corpus), isDemoFallback: false });
    } catch (error) {
      return NextResponse.json({ content: buildContent(idea), isDemoFallback: false, error: error instanceof Error ? error.message : "Source lookup failed." });
    }
  } catch (error) {
